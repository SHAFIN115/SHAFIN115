<div align="center">
<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&customColorList=0,2,2,5,30&height=3&section=header" width="100%"/>
</div>

<br/>

<div align="center">

# SHAFIN JUNAYED
### Software Engineer &nbsp;·&nbsp; Full-Stack &nbsp;·&nbsp; AWS Cloud

**3+ years** shipping production SaaS &nbsp;·&nbsp; **Japan-based** international team &nbsp;·&nbsp; **40+ AWS services** in production

<br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/shafin-junayed-b51480188/)
[![Gmail](https://img.shields.io/badge/Gmail-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:shafinjunayed954@gmail.com)
[![Medium](https://img.shields.io/badge/Medium-000000?style=flat-square&logo=medium&logoColor=white)](https://medium.com/@shafinjunayed954)
[![Stack Overflow](https://img.shields.io/badge/Stack_Overflow-F58025?style=flat-square&logo=stackoverflow&logoColor=white)](https://stackoverflow.com/users/12362769/shafin-junayed)
[![GitHub](https://img.shields.io/badge/GitHub-SHAFIN115-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/SHAFIN115)
&nbsp;
<img src="https://komarev.com/ghpvc/?username=SHAFIN115&style=flat-square&color=555555&label=Profile+Views" />

</div>

<br/>

---

## About

I design and maintain backend systems that power real businesses. At **JB Connect Ltd.**, I own multiple SaaS products end-to-end — from database schema evolution and service workflows to production debugging and cloud optimization on AWS. I've shipped integrations with Meta, Google, LINE, Yahoo, and Rakuten, and collaborate daily with Japanese engineers and product stakeholders to align engineering with business outcomes.

I go deep on my tools — Laravel, React, and AWS at production scale. I'm not a framework switcher.

<br/>

---

## Experience

<table width="100%">
<tr>
<td width="50%" valign="top">

**Software Engineer** &nbsp;&nbsp;`Jun 2023 – Present`
**JB Connect Ltd.** · Full-time

- Owns backend services & API integrations for multiple international SaaS products
- Designed multi-tenant database schemas and service workflows
- Led platform integrations: **Meta · Instagram Graph · Google APIs · LINE · Yahoo · Rakuten SMS**
- Built reusable React components with clean backend API contracts
- AWS workloads in production: EC2 · RDS · Lambda · S3 · SQS · SNS · CloudWatch
- Resolved high-impact production incidents, reducing downtime
- Daily collaboration with Japanese engineers across time zones

</td>
<td width="50%" valign="top">

**Intern Software Engineer** &nbsp;&nbsp;`Jun 2022 – May 2023`
**Nagorik Software Company** · Full-time

- Delivered **9 full-stack projects** using Laravel and React
- Implemented RESTful APIs, auth systems, and database logic from scratch
- Introduced practices adopted across the team's codebase

<br/>

**B.Sc. Computer Science & Engineering**
Khulna University of Engineering & Technology (KUET) &nbsp;`2018 – 2022`

<br/>

> *"I write backend code that runs reliably in production — not just locally."*

</td>
</tr>
</table>

<br/>

---

## Tech Stack

#### Backend & Databases
<p>
<img src="https://skillicons.dev/icons?i=laravel,php,mysql,python,nodejs" height="44"/>
</p>

`Laravel` &nbsp;`PHP` &nbsp;`MySQL` &nbsp;`Python` &nbsp;`Node.js` &nbsp;`RESTful APIs` &nbsp;`Eloquent ORM` &nbsp;`Queue Systems`

<br/>

#### Frontend
<p>
<img src="https://skillicons.dev/icons?i=react,vuejs,js,ts,html,css,bootstrap,tailwind" height="44"/>
</p>

`React` &nbsp;`Vue.js` &nbsp;`JavaScript` &nbsp;`TypeScript` &nbsp;`Reusable Component Architecture` &nbsp;`SPA`

<br/>

#### Cloud — AWS (40+ services in production)
<p>
<img src="https://skillicons.dev/icons?i=aws" height="44"/>
</p>

`EC2` &nbsp;`RDS` &nbsp;`Lambda` &nbsp;`S3` &nbsp;`SQS` &nbsp;`SNS` &nbsp;`CloudWatch` &nbsp;`API Gateway` &nbsp;`IAM` &nbsp;`VPC` &nbsp;`Route 53` &nbsp;`CloudFront` &nbsp;`ECS`

<br/>

#### DevOps & Tools
<p>
<img src="https://skillicons.dev/icons?i=docker,git,linux,postman,firebase,azure" height="44"/>
</p>

`Docker` &nbsp;`Git` &nbsp;`Linux` &nbsp;`Postman` &nbsp;`Jira` &nbsp;`CI/CD` &nbsp;`Firebase` &nbsp;`Azure`

<br/>

#### AI / ML
<p>
<img src="https://skillicons.dev/icons?i=tensorflow,opencv,pytorch" height="44"/>
</p>

`TensorFlow` &nbsp;`scikit-learn` &nbsp;`OpenCV` &nbsp;`AI Feature Integration` &nbsp;`Automation Pipelines`

<br/>

---

## Key Projects

<table width="100%">
<tr>
<td width="50%" valign="top">

**BridgeBooks (BBS) — JB Connect**
Backend ownership of a multi-client SaaS platform. Improved maintainability, service reliability, and introduced workflows that scaled with business growth.
`Laravel` `MySQL` `AWS` `Multi-tenant`

---

**[Workslayr — Tesla Portal](https://tesla.workslayr.dev)**
Designed and built the authentication system and management portal. Delivered UI/UX improvements and measurable frontend performance gains.
`React` `Laravel` `Auth` `UX`

---

**SEO SaaS Platform — Japan Market**
Engineered backend systems for account-location mapping and credit management. Built automated article generation workflows integrating multiple third-party services.
`Laravel` `AWS` `3rd-party APIs` `Automation`

</td>
<td width="50%" valign="top">

**SuperAnnotate AI/ML Platform**
Implemented AI-driven text extraction features and improved existing automation pipelines enabling faster data processing workflows.
`Python` `AI/ML` `Automation`

---

**Thailand-Based SaaS Product**
Contributed to core SaaS architecture, backend logic refinements, and performance optimization of high-traffic services.
`SaaS Architecture` `Laravel` `Performance`

---

**Gopalganj Medical College Website**
Led system design and requirement analysis. Delivered the full-stack platform with focus on scalability and long-term maintainability.
`Laravel` `MySQL` `System Design Lead`

</td>
</tr>
</table>

<br/>

---

## GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=SHAFIN115&show_icons=true&hide_border=true&title_color=24292f&icon_color=0969da&text_color=57606a&bg_color=f6f8fa&count_private=true&include_all_commits=true&hide=prs&custom_title=Stats&cache_seconds=1800" height="160"/>
&nbsp;&nbsp;
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SHAFIN115&layout=compact&hide_border=true&title_color=24292f&text_color=57606a&bg_color=f6f8fa&langs_count=8&custom_title=Top+Languages&cache_seconds=1800" height="160"/>

</div>

<div align="center">

<img src="https://streak-stats.demolab.com/?user=SHAFIN115&hide_border=true&background=f6f8fa&ring=0969da&fire=0969da&currStreakLabel=24292f&sideLabels=57606a&dates=57606a&stroke=ebebeb" height="160"/>

</div>

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=SHAFIN115&theme=flat&no-frame=true&column=7&margin-w=6&title=Stars,Commits,Repositories,Issues,Followers,PullRequest,Reviews&cache_seconds=1800" width="100%"/>

</div>

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=SHAFIN115&bg_color=f6f8fa&color=24292f&line=0969da&point=24292f&hide_border=true&custom_title=Contribution+Graph&cache_seconds=1800" width="100%"/>

</div>

<br/>

---

## Worldwide API Integrations & Business Partnerships

> I have contracted and integrated globally recognized third-party platforms into production SaaS systems — driving automation, business growth, and client satisfaction across international markets. Clients saw direct revenue and operational improvements from these integrations.

<br/>

#### Social & Messaging Platforms

<table width="100%">
<tr>
<td align="center" width="16%">

<img src="https://img.shields.io/badge/Meta_Graph_API-0866FF?style=for-the-badge&logo=meta&logoColor=white"/>

**Meta Graph API**
Posts · Insights · Ads automation

</td>
<td align="center" width="16%">

<img src="https://img.shields.io/badge/Instagram_Graph-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/>

**Instagram Graph**
Media · Analytics · Scheduling

</td>
<td align="center" width="16%">

<img src="https://img.shields.io/badge/LINE_Messaging-00C300?style=for-the-badge&logo=line&logoColor=white"/>

**LINE API**
Messaging · Notifications · Bots

</td>
<td align="center" width="16%">

<img src="https://img.shields.io/badge/Yahoo_API-6001D2?style=for-the-badge&logo=yahoo&logoColor=white"/>

**Yahoo API**
Search · Data feeds · Auth

</td>
<td align="center" width="16%">

<img src="https://img.shields.io/badge/Rakuten_SMS-BF0000?style=for-the-badge&logo=rakuten&logoColor=white"/>

**Rakuten SMS**
Transactional messaging · Japan market

</td>
<td align="center" width="16%">

<img src="https://img.shields.io/badge/Twitter_API-000000?style=for-the-badge&logo=x&logoColor=white"/>

**X (Twitter) API**
Feed · Post management

</td>
</tr>
</table>

<br/>

#### Google Ecosystem

<table width="100%">
<tr>
<td align="center" width="20%">

<img src="https://img.shields.io/badge/Google_APIs-4285F4?style=for-the-badge&logo=google&logoColor=white"/>

**Google APIs**
Core platform integrations

</td>
<td align="center" width="20%">

<img src="https://img.shields.io/badge/Google_My_Business-34A853?style=for-the-badge&logo=googlemybusiness&logoColor=white"/>

**Google My Business**
Location · Reviews · Listings

</td>
<td align="center" width="20%">

<img src="https://img.shields.io/badge/Google_Analytics-E37400?style=for-the-badge&logo=googleanalytics&logoColor=white"/>

**Google Analytics**
Traffic · Conversion tracking

</td>
<td align="center" width="20%">

<img src="https://img.shields.io/badge/Google_Search_Console-458CF5?style=for-the-badge&logo=googlesearchconsole&logoColor=white"/>

**Search Console**
Indexing · SEO signals

</td>
<td align="center" width="20%">

<img src="https://img.shields.io/badge/Google_Ads-4285F4?style=for-the-badge&logo=googleads&logoColor=white"/>

**Google Ads API**
Campaign · Reporting automation

</td>
</tr>
</table>

<br/>

#### SEO & Marketing Intelligence

<table width="100%">
<tr>
<td align="center" width="25%">

<img src="https://img.shields.io/badge/SEMrush-FF642D?style=for-the-badge&logo=semrush&logoColor=white"/>

**SEMrush API**
Keyword research · Rank tracking · Competitive analysis

</td>
<td align="center" width="25%">

<img src="https://img.shields.io/badge/SEO_Automation-222222?style=for-the-badge&logo=googlesearchconsole&logoColor=white"/>

**SEO Pipeline Automation**
Article generation · Account-location mapping · Content workflows

</td>
<td align="center" width="25%">

<img src="https://img.shields.io/badge/Credit_Management-0052CC?style=for-the-badge&logo=stripe&logoColor=white"/>

**Credit & Billing Systems**
Usage-based credit tracking · Quota enforcement · Client billing logic

</td>
<td align="center" width="25%">

<img src="https://img.shields.io/badge/Multi--Platform_Publishing-FF6B35?style=for-the-badge&logo=buffer&logoColor=white"/>

**Multi-Platform Publishing**
Cross-channel post scheduling · Analytics aggregation

</td>
</tr>
</table>

<br/>

#### What These Integrations Delivered

```
✦  Contracted directly with globally recognized platforms on behalf of international clients
✦  Transformed manual business workflows into fully automated, API-driven pipelines
✦  Reduced client operational overhead significantly — measurable time and cost savings
✦  Enabled businesses to manage Meta, Instagram, Google, LINE, Yahoo & Rakuten from a single SaaS dashboard
✦  Supported SEO growth strategies through automated content generation and location-based mapping
✦  Provided business logic consulting — proposed and shipped features aligned with revenue growth
✦  Clients reported direct business growth and operational improvements post-integration
```

<br/>

---

## Competitive Programming

<table width="100%">
<tr>
<td align="center" width="25%">

[![Codeforces](https://img.shields.io/badge/Codeforces-1F8ACB?style=for-the-badge&logo=codeforces&logoColor=white)](https://codeforces.com/profile/shafinjunayed954)

**230+ Problems**
Multi-language

</td>
<td align="center" width="25%">

[![HackerRank](https://img.shields.io/badge/HackerRank-00EA64?style=for-the-badge&logo=hackerrank&logoColor=black)](https://www.hackerrank.com/shafinjunayed954)

**Gold — C++**
Silver — SQL

</td>
<td align="center" width="25%">

[![LeetCode](https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=white)](https://leetcode.com/toynbee/)

**DSA Practice**
Algorithms & Data Structures

</td>
<td align="center" width="25%">

![ACM ICPC](https://img.shields.io/badge/ACM_ICPC-00599C?style=for-the-badge&logoColor=white)

**Regional Participant**
Competitive Programming

</td>
</tr>
</table>

<br/>

---

## Certifications

<table width="100%">
<tr>
<td align="center" width="25%">

![Git](https://img.shields.io/badge/Git_%26_GitHub-F05032?style=flat-square&logo=git&logoColor=white)

**Git & GitHub**

</td>
<td align="center" width="25%">

![SQL](https://img.shields.io/badge/SQL_Databases-4479A1?style=flat-square&logo=mysql&logoColor=white)

**SQL Databases**

</td>
<td align="center" width="25%">

![Jira](https://img.shields.io/badge/Jira-0052CC?style=flat-square&logo=jira&logoColor=white)

**Jira · Project Management**

</td>
<td align="center" width="25%">

![Coursera](https://img.shields.io/badge/Coursera-0056D2?style=flat-square&logo=coursera&logoColor=white)

**Machine Learning · Coursera**

</td>
</tr>
</table>

<br/>

---

## Connect With Me

<div align="center">

Open to senior engineering roles, technical collaborations, and interesting backend or cloud architecture challenges.

<br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/shafin-junayed-b51480188/)
[![Gmail](https://img.shields.io/badge/Email-Reach_Out-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:shafinjunayed954@gmail.com)
[![Medium](https://img.shields.io/badge/Medium-Read_Blog-000000?style=for-the-badge&logo=medium&logoColor=white)](https://medium.com/@shafinjunayed954)
[![Stack Overflow](https://img.shields.io/badge/Stack_Overflow-Community-F58025?style=for-the-badge&logo=stackoverflow&logoColor=white)](https://stackoverflow.com/users/12362769/shafin-junayed)
[![Kaggle](https://img.shields.io/badge/Kaggle-Data_Science-20BEFF?style=for-the-badge&logo=kaggle&logoColor=white)](https://www.kaggle.com/shafinjunayed)
[![Twitter](https://img.shields.io/badge/Twitter-Follow-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white)](https://twitter.com/shafinjunayed2)

<br/>

`shafinjunayed954@gmail.com` &nbsp;&nbsp;·&nbsp;&nbsp; `Dhaka, Bangladesh`

</div>

<br/>

<div align="center">
<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&customColorList=0,2,2,5,30&height=3&section=footer" width="100%"/>
</div>
